...
class TestInsert:
    def register_jobs(self, job_names: List[str]) -> List[int]:
        with db_session:
            ...
            db_job_ids = select(j.id for j in models.Job)[:]
    ...
    def test_normal(self, fixt_init_db: None, end: datetime | None) -> None:
...
class TestUpdate:
    def register_job_records(self, job_names: List[str]) -> Tuple[List[int], List[int]]:
        with db_session:
            ...
            db_job_ids = select(j.id for j in models.Job)[:]
            db_job_record_ids = select(jr.id for jr in models.JobRecord)[:]

        return db_job_ids, db_job_record_ids

    @pytest.mark.parametrize(("replaced_end"), [(datetime.now()), (None)])
    def test_normal(self, fixt_init_db: None, replaced_end: datetime | None) -> None:
    ...
    @pytest.mark.parametrize(("replaced_end"), [(datetime.now()), (None)])
    def test_end_normal(
        self, fixt_init_db: None, replaced_end: datetime | None
    ) -> None:
...
class TestSelect:
    def register_job_records(
        ...
        with db_session:
            ...
            db_job_record_attrs = select(
                (jr.id, jr.job.id, jr.start, jr.end) for jr in models.JobRecord
            )[:]
    ...
    def validate_all_finished_by_date(
        self,
        __date: date,
        expected_job_record_attrs: List[Tuple[int, int, datetime, datetime]],
    ) -> None:
        ...
        with db_session:
            ...
            for i in range(len(db_job_records)):
                ...
                assert db_job_records[i].end == sorted_expected_job_record_attrs[i][3]
    ...
    def test_all_finished_by_date_normal_date_empty(self, fixt_init_db: None) -> None:
        self.validate_all_finished_by_date(CURRENT_DATE, [])

    def test_all_finished_by_date_normal_data_exists(self, fixt_init_db: None) -> None:
    ...
    def test_all_finished_by_date_data_missing_for_in_progress(
        self, fixt_init_db: None
    ) -> None:
        self.register_job_records(JOB_NAMES, CURRENT_DATE, False)
        self.validate_all_finished_by_date(CURRENT_DATE, [])

    def test_all_finished_by_date_data_missing_for_wrong_dates(
        self, fixt_init_db: None
    ) -> None:
    ...
    def test_one_in_progress_by_date_normal_data_empty(
        self, fixt_init_db: None
    ) -> None:
        self.validate_one_in_progress_by_date(CURRENT_DATE, None, None, None, True)

    def test_one_in_progress_by_date_normal_data_exists(
        self, fixt_init_db: None
    ) -> None:
    ...
    def test_one_in_progress_by_date_normal_data_missing(
        self, fixt_init_db: None
    ) -> None:
